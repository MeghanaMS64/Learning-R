...
# 3. The "weight" column has the incorrect data type.
...
# Find the data type of the names of mean_male_height
...
```{r}
# Explain the outputs of the following commands:

# 1. Returns a data frame with the first column
typeof(cats[1])

# 2. Returns a vector with the first column
typeof(cats[[1]])

# 3. Returns a vector with the "coat" column
typeof(cats$coat)

# 4. Returns a data frame with the "coat" column
typeof(cats["coat"])

# 5. Returns the element at the first row and first column
typeof(cats[1, 1])

# 6. Returns a vector with the first column
typeof(cats[, 1])

# 7. Returns a data frame with the first row
typeof(cats[1, ])
...
```         
length(matrix_example) # The result will be 18 because it's the total number of elements in the matrix. 
```
...
```{r}
# Using matrix with default parameters (fills by column)
matrix1 <- matrix(1:50, ncol = 5, nrow = 10)

# Modifying the previous command to fill by row
matrix2 <- matrix(1:50, ncol = 5, nrow = 10, byrow = TRUE)
...
```{r}
# Create a list of length two
my_list <- list(
  data_types = c("double", "integer", "complex", "logical", "character"),
  data_structures = c("vector", "list", "matrix", "data.frame")
)
