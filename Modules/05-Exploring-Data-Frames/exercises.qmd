...
```{r}
# Load the cats data (make sure you are in the correct working directory)
...
# 3. Convert human_age back to a numeric vector using as.numeric() and divide by 7 to get the original ages back
human_age <- as.numeric(human_age) / 7

```

Explanation: We initially multiplied ages by 7 to get human ages and converted them to a factor. When we converted them back to numeric, R assigned integer values to the levels of the factor. Dividing by 7 gives us the original ages.
...
```{r}
# Create a data frame for yourself
yourself <- data.frame(
  first_name = "Meghana",
  last_name = "Sripathi",
  lucky_number = 7,
...
# Create a data frame for a team member
...
people <- rbind(yourself, team_member)

# Add a column with answers to the coffee break question
people$coffee_break_answer <- c("Yes", "No")
...
# Check some lines in the middle (e.g., lines 10 to 15)
gapminder[10:15, ]

random_rows <- gapminder[sample(nrow(gapminder), 5), ]
...
-   country: A factor with 142 levels representing the countries in the dataset.

-   continent: A factor with 5 levels representing the continents.

-   year: An integer vector representing the years.

-   lifeExp: A numeric vector representing life expectancy.

-   pop: A numeric vector representing population.

-   gdpPercap: A numeric vector representing GDP per capita.
