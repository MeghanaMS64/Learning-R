...
is_seAsia <- countries == seAsia
# clunky way to subset `countries`
is_seAsia <- countries == seAsia[1] | countries == seAsia[2] | countries == seAsia[3] | countries == seAsia[4] | countries == seAsia[5]

# elegant way to subset `countries`
is_seAsia <- countries %in% seAsia
...
# your answer goes here
mod <- aov(pop ~ lifeExp, data=gapminder)
residual_df <- attributes(mod)$df.residual
...
gapminder\[1:20\] returns an error because it is attempting to subset rows only, not specifying columns. gapminder\[1:20, \] includes all columns and the first 20 rows.
